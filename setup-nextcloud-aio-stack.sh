...
read -r -p "Открыть панель Nextcloud AIO (порт 8080) на все интерфейсы, а не только localhost? [y/N]: " AIO_PUBLIC
AIO_PUBLIC="${AIO_PUBLIC,,}"
if [[ "$AIO_PUBLIC" == "y" || "$AIO_PUBLIC" == "yes" ]]; then
  AIO_BIND="0.0.0.0"
  warn "Панель AIO будет доступна всем на порту 8080. Она защищена собственным паролем, но лучше ограничить доступ файрволом."
else
  AIO_BIND="127.0.0.1"
